...
#include "tthAnalysis/tthMEM/interface/mg5/helamps/HelAmps_sm_tth_t2tavt_tbar2lvl_h2WW_Wp2lvl_Wn2tavt.h"
#include "tthAnalysis/tthMEM/interface/Exception.h" // throw_line_ext()

using namespace std; 

namespace MG5_sm_tth_t2tavt_tbar2lvl_h2WW_Wp2lvl_Wn2tavt
{
    ...
    double M1, double W1, complex<double> V1[])
{
  ...
  denom = COUP/(pow(P1[0], 2) - pow(P1[1], 2) - pow(P1[2], 2) - pow(P1[3], 2) -
      M1 * (M1 - cI * W1));
  V1[2] = denom * (TMP9 * (-cI * (P2[0]) + cI * (P3[0])) + (V2[2] * (-cI *
      (TMP5) + cI * (TMP6)) + V3[2] * (-cI * (TMP8) + cI * (TMP7))));
  V1[3] = denom * (TMP9 * (-cI * (P2[1]) + cI * (P3[1])) + (V2[3] * (-cI *
      (TMP5) + cI * (TMP6)) + V3[3] * (-cI * (TMP8) + cI * (TMP7))));
  V1[4] = denom * (TMP9 * (-cI * (P2[2]) + cI * (P3[2])) + (V2[4] * (-cI *
      (TMP5) + cI * (TMP6)) + V3[4] * (-cI * (TMP8) + cI * (TMP7))));
  V1[5] = denom * (TMP9 * (-cI * (P2[3]) + cI * (P3[3])) + (V2[5] * (-cI *
      (TMP5) + cI * (TMP6)) + V3[5] * (-cI * (TMP8) + cI * (TMP7))));
}
